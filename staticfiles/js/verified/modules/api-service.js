...
const ApiService = (function() {
    ...
    async function handleResponseError(response) {
        ...
        throw new Error(`HTTP error! status: ${response.status}. ${String(responseData || '').slice(0, 200)}`);
    }
    ...
    async function post(url, data = {}, timeout = DEFAULT_TIMEOUT) {
        const token = typeof Security !== 'undefined' ? Security.getCSRFToken() : (csrftoken || Utils.getCSRFToken());
    ...
    async function fetchUserOffice(forceRefresh = false) {
        ...
        return deduplicateRequest(cacheKey, async () => {
            try {
                ...
                console.warn('Error getting user office (non-critical):', error.message || error);
                return '';
            }
        });
    }
    ...
    async function fetchApiKeys(forceRefresh = false) {
        ...
        return deduplicateRequest(cacheKey, async () => {
            try {
                ...
                // Gracefully handle 404 or other errors for optional endpoint
                console.warn('Error getting API keys (non-critical):', error.message || error);
                return { api_dongvan: '', api_fmail: '', api_phapsu: '' };
            }
        });
    }
    ...
    async function getReserveMailCount(forceRefresh = false) {
        ...
        return deduplicateRequest(cacheKey, async () => {
            try {
                ...
                console.warn('Reserve mail count not available:', data?.error || 'No data');
                return { success: false, count: 0 };
            } catch (error) {
                // Gracefully handle errors for this optional endpoint
                console.warn('Error getting reserve mail count (non-critical):', error.message || error);
                return { success: false, count: 0 };
            }
        });
    }
    ...
    async function getDongVanBalance(forceRefresh = false) {
        ...
        return deduplicateRequest(cacheKey, async () => {
            const data = await get('/api/dongvan-balance/', { apikey: apiData.api_dongvan });
            setCache(cacheKey, data);
            return data;
        });
    }
    ...
    async function getFMailBalance(forceRefresh = false) {
        ...
        return deduplicateRequest(cacheKey, async () => {
            const data = await get('/api/fmail-balance/', { apikey: apiData.api_fmail });
            setCache(cacheKey, data);
            return data;
        });
    }
    ...
    async function getPhapSuBalance(forceRefresh = false) {
        ...
        return deduplicateRequest(cacheKey, async () => {
            const data = await get('/api/phapsu-balance/', { apikey: apiData.api_phapsu });
            setCache(cacheKey, data);
            return data;
        });
    }
    ...
    async function buyFMail(id, amount) {
        ...
        return postUrlEncoded(`/api/fmail-buy/?apikey=${apiData.api_fmail}`, { id, amount });
    }
    ...
    async function buyPhapSu(id, amount) {
        ...
        return postUrlEncoded(`/api/phapsu-buy/?apikey=${apiData.api_phapsu}`, { id, amount });
    }
            ...
            promise
                .then(data => {
                    if (data.success && data.data?.money !== undefined) {
                        badge.textContent = data.data.money.toLocaleString('vi-VN') + ' VND';
                    } else {
                        badge.textContent = '?';
                    }
                })
