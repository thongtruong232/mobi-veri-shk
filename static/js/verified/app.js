...
if (typeof ErrorHandler !== 'undefined') {
    ...
    ErrorHandler.setGlobalErrorCallback((error, context) => {
        console.error('[App] Global error caught:', error.getUserMessage(), context);
...
const AppState = {
    get userOffice() { return StateManager.getState('userOffice'); },
    set userOffice(val) { StateManager.actions.setUserOffice(val); },
    
    get apiData() { return StateManager.getState('apiData'); },
    set apiData(val) { StateManager.actions.setApiData(val); },
    
    get csrftoken() { return StateManager.getState('csrftoken'); },
    set csrftoken(val) { StateManager.actions.setCsrfToken(val); },
    
    get recordsToday() { return StateManager.getState('records'); },
    set recordsToday(val) { StateManager.actions.setRecords(val); },
    
    get isTextNowStatus() { return StateManager.getState('isTextNowStatus'); },
    set isTextNowStatus(val) { StateManager.setState('isTextNowStatus', val); },
    
    get initialized() { return StateManager.getState('initialized'); },
    set initialized(val) { StateManager.actions.setInitialized(val); }
};
...
function renderEditStatusOptions() {
    ...
    const options = isEditingTextNow
        ? ['not reg', 'verified', 'error', 'resend link', 'success', 'other password']
    ...
    statusSelect.value = isEditingTextNow ? editModal.statusTN : editModal.statusTF;
}
...
async function saveEdit() {
    ...
    const btnState = saveButton ? UIHelpers.setButtonLoading(saveButton, 'Saving...') : null;

    try {
        const isEditingTextNow = editModal.isEditingTextNow;
        const statusField = isEditingTextNow ? 'status_account_TN' : 'status_account_TF';
        const data = await ApiService.updateTextNowStatus(editModal.email, statusField, newStatus);

        if (data.success) {
            UIHelpers.showToast('Updated status successfully!', 'success');
            
            setTimeout(() => {
                TableManager.updateRowStatus(editModal.email, newStatus, isEditingTextNow ? 'TN' : 'TF');
                searchRecords();
            }, 500);
        } else {
            const errorMsg = 'Error when updating: ' + (data.error || 'Unknown error');
...
function toggleStatusLabel() {
    ...
    if (isTextNowStatus) {
        if (statusLabel) statusLabel.textContent = 'TextNow Status: (click to switch)';
        if (statusSelect) {
            statusSelect.innerHTML = `
        ...
        if (statusLabel) statusLabel.textContent = 'TextFree Status: (click to switch)';
        if (statusSelect) {
            statusSelect.innerHTML = `
                <option value="">All</option>
                <option value="success">Success</option>
...
// Mail operations
window.showReserveMailModal = MailOperations.showReserveMailModal;
window.closeReserveMailModal = MailOperations.closeReserveMailModal;
window.confirmGetReserveMail = MailOperations.confirmGetReserveMail;
window.showMailListModal = MailOperations.showMailListModal;
window.closeMailListModal = MailOperations.closeMailListModal;
window.closePurchaseModal = MailOperations.closePurchaseModal;
...
async function initializeApp() {
    ...
    console.log('[App] Initializing verified page (Phase 4: State Management)...');
    ...
    try {
        ...
        try {
            ...
            console.warn('[App] Error fetching user office (non-critical):', error.message || error);
        }
        ...
        try {
            ...
            console.warn('[App] Error fetching API keys (non-critical):', error.message || error);
        }

        // Update API balances
        ApiService.updateAllBalances();

        // Initialize reserve mail count
        MailOperations.updateReserveMailCount();
        setInterval(() => MailOperations.updateReserveMailCount(), 60000);

        // Set default search date and update state
        const searchDateInput = document.getElementById('searchDate');
        if (searchDateInput && !searchDateInput.value) {
            const today = Utils.getTodayDateString();
            searchDateInput.value = today;
            StateManager.actions.setFilters({ date: today });
        }
...
function setupStateSubscriptions() {
    ...
    StateManager.subscribe('isTextNowStatus', (value) => {
        console.log('[StateManager] Status type changed:', value ? 'TextNow' : 'TextFree');
    });
}
...
function setupEventListeners() {
    ...
    if (searchDate) {
        searchDate.addEventListener('change', debouncedSearch);
    }
    if (statusTN) {
    ...
    if (getReserveMailBtn) {
        getReserveMailBtn.addEventListener('click', () => {
            MailOperations.showReserveMailModal();
        });
    }
    ...
    if (importMailBtn) {
        importMailBtn.addEventListener('click', (e) => {
            e.preventDefault();
            MailOperations.showImportOptionsModal();
        });
    }
    ...
    if (deleteAllEmailsBtn) {
        deleteAllEmailsBtn.addEventListener('click', () => {
            MailOperations.deleteAllEmails();
        });
    }
    ...
    if (buyDongVanBtn) {
        buyDongVanBtn.addEventListener('click', () => {
            MailOperations.buyFromSupplier('dongvan');
        });
    }

    const buyFMailBtn = document.getElementById('buyFMailBtn');
    if (buyFMailBtn) {
        buyFMailBtn.addEventListener('click', () => {
            MailOperations.buyFromSupplier('fmail');
        });
    }

    const buyPhapSuBtn = document.getElementById('buyPhapSuBtn');
    if (buyPhapSuBtn) {
        buyPhapSuBtn.addEventListener('click', () => {
            MailOperations.buyFromSupplier('phapsu');
        });
    }
